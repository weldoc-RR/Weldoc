...
const CreateControleSchema = z
  .object({
    ...
    // produits ressaisis ici (ex. film radiographique, produit de développement).
    consommableIds: z.array(z.string()).optional(),
    signatureId: z.string().optional(),
  })
  .merge(ConditionsExamenSchema);

// GET /api/controles-radiographie?jointId=...
...
// POST /api/controles-radiographie (RT) — même principe que le contrôle
...
export async function POST(req: NextRequest) {
  ...
  const controle = await prisma.controleRadiographie.create({
    data: {
      jointId,
  ...
  for (const consommableId of consommableIds ?? []) {
    await prisma.controleRadiographieConsommable.create({
      data: { controleRadiographieId: controle.id, consommableId },
    });
  }
  ...
  if (resultat === "NON_CONFORME") {
    fnc = await prisma.fNC.create({
      data: {
        ...
        jointId,
        controleRadiographieOrigineId: controle.id,
        description: `Contrôle par radiographie non conforme sur ${joint.numero} (${procedureRef}).`,
        impact: "BLOQUANTE",
        statut: "DETECTION",
      },
    });
