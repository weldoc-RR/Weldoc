...
// et l'entreprise/service saisis au moment de signer — POST
// /api/phases/signatures-detaillees. Le nom de l'intervenant se remplit
// tout seul à la signature (identification QR/matricule + code PIN),
// jamais saisi à la main. Une même personne peut apparaître plusieurs fois
// pour des fonctions différentes ; chaque ligne est une signature réelle,
...
export function SignaturesDetailleesPhase({ phaseId, signatures }: { phaseId: string; signatures: SignatureDetaillee[] }) {
  ...
  async function signer(e: React.FormEvent) {
    ...
    const res = await fetch("/api/phases/signatures-detaillees", {
      ...
      body: JSON.stringify({ phaseId, fonction, habilitation, entrepriseService, identifiant, pin }),
    });
