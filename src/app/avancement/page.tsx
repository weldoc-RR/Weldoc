      ...
      {avancements.length === 0 ? (
        ...
        <ul style={{ listStyle: "none", padding: 0 }}>
          {avancements.map(({ affaire, avancement }) => (
            <li
              key={affaire.id}
              style={{ marginBottom: "1rem", borderBottom: "1px solid var(--couleur-bordure)", paddingBottom: "0.75rem" }}
            >
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginTop: "0.35rem" }}>
                <div style={{ width: 320, height: 10, background: "var(--couleur-fond-discret)", borderRadius: 5, overflow: "hidden" }}>
                  <div
                    style={{
                      width: `${avancement.pourcentageGlobal}%`,
                      height: "100%",
                      background: "var(--couleur-conforme)",
                    }}
                  />
                </div>
                ...
                {avancement.fnc.ouvertes > 0 && (
                  <span style={{ color: "var(--couleur-non-conforme)" }}>{avancement.fnc.ouvertes} FNC ouverte(s)</span>
                )}
              </div>
            </li>
          ))}
